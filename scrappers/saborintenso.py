    ...
    for i in categories:
        ...
        for a in range(1, calculateNumberOfPages(tree)):
            ...
            for k in range(len(recipe_name)):
                ...
                finalRecipes.append({"category": category, "name": recipe_name[k], "img": recipe_img[k], "ingredients_recipe": getIngredientsForMobile(ingredientsAndPrep[0][k]), "prep": ingredientsAndPrep[1][k]})
                try:
                  result = db.saborintenso.insert_many(finalRecipes, ordered=False)
